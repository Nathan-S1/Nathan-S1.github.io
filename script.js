...
const projects = {
  ...
  tetrisai: {
    ...
    description:
      "An AI designed to play Tetris with readable visual output. It includes normal animated and accelerated drop modes.",
  ...
  imagegenerator: {
    ...
    description:
      "A Stable Diffusion image-generation project that opens its notebook in Google Colab for Python and GPU processing.",
  ...
  videogenerator: {
    ...
    description:
      "A diffusion-based video-generation project that opens its notebook in Google Colab for Python and GPU processing.",
  ...
  texttospeech: {
    ...
    description:
      "A browser tool that converts entered text into spoken audio using voices installed in the visitor's browser and operating system.",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const summary =
    byId("project-summary");

  const selectedType =
    byId("selected-project-type");

  const selectedTitle =
    byId("selected-project-title");

  const selectedDescription =
    byId("selected-project-description");

  const loadButton =
    byId("load-project-button");

  const closeButton =
    byId("close-project-button");

  const message =
    byId("project-message");
  ...
  const loading =
    byId("project-loading");

  const activeTitle =
    byId("active-project-title");

  const activeType =
    byId("active-project-type");

  const activeRepo =
    byId("active-project-repo");

  const fullscreenButton =
    byId("fullscreen-project-button");
  ...
  fullscreenButton.addEventListener("click", () => {
    if (!activeProjectUrl) {
      message.textContent =
        "Load a project before opening it in a new tab.";
